...
| Forth<br/>(zForth — default) | `WF_FORTH_ENGINE=zforth` | ~7 KB (core + WF wrapper) | ~66 KB (`ZF_DICT_SIZE=65536` = 64 KB dict + stacks per WF `zfconf.h`) |
...
| *Concatenative stack language; scripts compile to a compact bytecode dictionary at load time. WF's zForth build uses `ZF_DICT_SIZE=65536` (64 KB dictionary) and **4-byte float cells** (`ZF_CELL=float`, `zfconf.h:24`) — so `/` is floating-point division, **not** integer; use `mod` (or its native alias `%`) for integer remainder and round explicitly where you need integers. Dictionary pre-loaded with `INDEXOF_*` constants, the `read-mailbox` / `write-mailbox` / `write-actor-mailbox` host words, and a standard ANS CORE word set — `0= 0< 0> negate abs min max ?dup nip tuck rot -rot 2dup 2drop 2swap +! mod`, plus `if/else/then`, `begin/until`, `do/loop`, the comparisons, and `1+ 1- over` (defined in `engine/stubs/scripting_zforth.cc` `kCoreBootstrap`; see [`docs/level-building.md` § Scripting System](level-building.md#scripting-system) for the full vocabulary). zForth renames a few standard words: `&`=`and`, `|`=`or`, `<0`=`0<`, `%`=`mod` — writing `and`/`or`/`mod`-via-`/` will misbehave. No GC; no heap allocation after init — real-time safe. `WF_FORTH_ENGINE` is a compile-time backend switch; the same Forth source runs unchanged on all backends. embed note: on PC dev, mailbox float values are truncated to 16-bit int; correct on the real fixed-point target. pForth note: loads `system.fth` from `engine/vendor/pforth-*/fth/` at init to build the full ANS word set (IF/ELSE/THEN, DO/LOOP, etc.). Runtime memory column (`—`) pending measured values.* | | | |
...
| Forth (zForth) | <pre>INDEXOF_HARDWARE_JOYSTICK1_RAW read-mailbox INDEXOF_INPUT write-mailbox</pre> | <pre>: ?cam ( mb -- ) read-mailbox ?dup if INDEXOF_CAMSHOT write-mailbox then ;&#10;100 ?cam   99 ?cam   98 ?cam</pre> |
...
- **Forth (zForth)**: no native coroutine. Use `dup N = if ... fi` dispatch
  ...
  (`then` also works — both are defined). Engine-level YIELD (snapshot/restore of
  `zf_ctx` across ticks) would enable linear coroutine syntax and is a future feature.
  **Available words** (see `docs/level-building.md` § Scripting System for the full
  list): `0= 0< 0> 0<>` (zero tests), `negate abs min max mod`, `?dup nip tuck
  -rot 2dup 2drop 2swap +!`, `1+` / `1-`. Use `read-mailbox if` for a direct
  non-zero test (value consumed by `if`); `?dup if ... then` when you need the value
  on the true branch only (`?dup` duplicates non-zero, leaves 0 unchanged); `dup 0= if
  drop ... else ... then` when you need the value on both branches. `0 <>` before `if`
  is never needed just to branch — `if` treats any non-zero as true.

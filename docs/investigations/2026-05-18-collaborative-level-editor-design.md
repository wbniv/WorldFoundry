...
- **Save** = walk `Y.Doc` → [levtree-rs](../../wftools/levtree-rs/) canonical printer emits `.lev` → `build_level_binary.sh` (iffcomp-rs → levcomp-rs → textile-rs) / wf_blender exporter handles packaging and `.blend` round-trip. **v1 trigger is a manual "Publish" button** — the slow Blender round-trip happens only when the user explicitly asks for it. Whether to add autosave-on-idle, Ctrl-S, or every-Nth-edit triggers is a question for after v1 ships, when actual usage patterns reveal what's wanted. **(Landed 2026-05-21; see [plan](../plans/2026-05-21-editor-save-roundtrip.md).)** File→Save (Ctrl+S) writes the `.lev` and "Save + Compile" runs the build to `.iff`; both shipped. Save is a **pure `Doc → JSON` walk** — the Doc is lossless (v2 schema: each leaf's literals stored as a structured `items` array, kinds intact; [plan](../plans/2026-05-21-lossless-doc-schema.md), landed 2026-05-21), so there's no retained-JSON side-channel and structural/remote edits save correctly. (The save round-trip first shipped patching a retained `levtree parse` JSON to sidestep the then-lossy loader; the lossless schema removed that.) On top of this the Outliner ships actor **add/delete** (Duplicate / Delete + Del key; [plan](../plans/2026-05-21-outliner-add-delete.md), 2026-05-21) — structural edits persist on save; reflecting them in the *live viewport* without a reload (and a stable-id replacement for the bridge's positional identity map) is a logged follow-up. `.blend` round-trip uses the existing `wf.import_level` plugin (re-import), not a new editor exporter.

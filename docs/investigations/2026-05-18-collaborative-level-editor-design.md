...
- **Save** = walk `Y.Doc` → [levtree-rs](../../wftools/levtree-rs/) canonical printer emits `.lev` → `build_level_binary.sh` (iffcomp-rs → levcomp-rs → textile-rs) / wf_blender exporter handles packaging and `.blend` round-trip. **v1 trigger is a manual "Publish" button** — the slow Blender round-trip happens only when the user explicitly asks for it. Whether to add autosave-on-idle, Ctrl-S, or every-Nth-edit triggers is a question for after v1 ships, when actual usage patterns reveal what's wanted. **(Landed 2026-05-21; see [plan](../plans/2026-05-21-editor-save-roundtrip.md).)** File→Save (Ctrl+S) writes the `.lev` and "Save + Compile" runs the build to `.iff`; both shipped. One refinement vs. the sketch above: the Doc loader is *lossy* (collapses leaf literals to space-joined text), so save **patches the retained `levtree parse` JSON** with the Doc's edits rather than reconstructing JSON from the Doc — correct for v1's value-only single-writer edits; a [lossless Doc schema](../../TODO.md) (literals as a structured array) is the follow-up for structural/remote save. `.blend` round-trip uses the existing `wf.import_level` plugin (re-import), not a new editor exporter.

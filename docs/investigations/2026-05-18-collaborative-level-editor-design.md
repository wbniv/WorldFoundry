...
- **Engine linkability — Phase 0a DONE (commit `d865c40`, 2026-05-18); Phase 0b in progress, ~3 weeks remaining.** Today `wf_game` is a single `add_executable` target ([CMakeLists.txt](../../CMakeLists.txt) line 444) — every engine `.cc` file (~150+, 22 MB debug binary) compiles into the one executable.
  ...
  **Phase 0b — embed-readiness, before serious editor work. Four surgical changes, ~3 weeks total.** Sub-task order is "cheapest first" so partial progress remains useful.
    1. **Frame-step API** (~1–2 weeks). Split `WFGame::RunLevel`'s `while` body out into `Level::Step(dt)` / `WFGame::StepFrame(dt)`. The loop itself moves to the editor. The inner body touches `HALPumpSuspendedEvents` / `HALWindowCloseRequested` / suspend handling, which want untangling alongside.
    2. **Externally-supplied GL context** (~1 week). Add an `InitWithExistingContext(Display*, Window, GLXContext)` alternative path to [gfx/gl/mesa.cc](../../wfsource/source/gfx/gl/mesa.cc) that skips `XOpenDisplay` / `glXCreateContext` / `XCreateWindow`. The standalone path stays for `wf_game`.

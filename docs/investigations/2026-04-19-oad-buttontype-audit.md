...
1. **Int32 enum-label lookup misses when DATA is binary** — uncovered 2026-04-19 (obj[12], obj[35] in snowgoons LVL). `int_value` calls `field_str_value(chunk)`, which prefers the `DATA` sub-chunk. For `I32`-typed fields the DATA is 4 binary bytes; `read_cstr` on `00 00 00 00` returns `""`, and `Option::or` treats `Some("")` as present — so we never fall through to the nested `STR "Track"`. Result: `int_value` returns `0` from the data path instead of `1` from the enum path.
   **Fix**: add a `field_str_child_only` accessor that only reads nested `STR` children, ignoring `DATA`. Use it in `int_value`'s enum path.

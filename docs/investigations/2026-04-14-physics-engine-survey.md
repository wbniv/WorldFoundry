...
- **Measuring the replacement.** Before and after: `size wf_game` stripped with `WF_PHYSICS_ENGINE=legacy` vs `=jolt`, plus RSS peak during a snowgoons play-through. Publish next to the scripting-engine size table in [docs/scripting-languages.md](../scripting-languages.md) (or a new [docs/engine-size-matrix.md](../engine-size-matrix.md)).

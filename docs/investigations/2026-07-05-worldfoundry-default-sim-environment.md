...
Three observations, substantially revised by the 2026-07-07 [competitive](../investigations/2026-07-07-competitive-landscape.md) and [demand](../investigations/2026-07-07-game-worlds-vtt-demand.md) passes — and the revision matters: **(a)** the hard part is CRDT simultaneous editing, not A/V (Figma won without A/V; built-in voice/video is a neutralized non-factor). **(b) But this quadrant is no longer vacant.** [PlayCanvas](https://playcanvas.com/) (Snap-owned) already ships browser + real-time-collaborative + game-*engine* editing; [Spline](https://spline.design/) holds browser-collab-3D in design; Tabula Sono holds browser-3D-VTT-*play*. Real-time collaborative browser 3D is **commoditized**, not WorldFoundry's edge. **(c) Nor is it strongly *demanded* for structured building** — pros use version control (Unity killed Collaborate for Plastic SCM; even Unreal's live co-editing is positioned as an *augment* to source control), most VTT GMs prep solo, and real-time co-building clearly wins only in the casual Minecraft sandbox. **So the moat is not the collaborative editor — it is the engine** (fixed-point, tiny, everywhere, retro/embedded export), exactly as §2 and the "footprint + determinism = one moat" argument already said. The genuinely-vacant slot is that **engine combination**, not the collaboration — which *raises* the stakes on the moat×pull map below (the browser-collab advantage just dropped in both moat and pull).
...
quadrantChart
    ...
    "Browser collab editing": [0.40, 0.38]
...
Reading across the quadrants tells the whole prioritization: the deepest moats — **determinism** and **no-FPU embedded reach** — sit top-left in *"moat without proven demand"*: unforgeable, but the market hasn't asked for them yet, so protect and seed them rather than building the pitch around them. **glTF interop** is bottom-right *table stakes*: strong pull, zero moat — it's the monetization doc's #1 unlock, yet it differentiates nothing. **Voice/video** sits low: as a differentiator it's commodity WebRTC with (now confirmed) no pull (keep it — cheap and expected — just don't lead with it). And **browser collaborative editing has dropped in both moat and pull** *(revised 2026-07-07)*: the [competitive pass](../investigations/2026-07-07-competitive-landscape.md) showed it's **commoditized** (PlayCanvas/Spline/Tabula Sono), gutting the moat, and the [demand pass](../investigations/2026-07-07-game-worlds-vtt-demand.md) showed it's **weakly demanded** for structured building (VCS wins; solo prep is the norm), gutting the pull — so it slides from a near-winner to the low-left. That leaves the real instruction: **lead with the engine.** The determinism + no-FPU/embedded-reach + retro export is the only genuinely defensible, un-commoditized ground — ship glTF/URL-worlds as table-stakes to open the funnel, but the pitch and the moat are the *engine that runs everywhere and ships to hardware nobody else can reach*, not the collaborative editor.

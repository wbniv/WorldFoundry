...
Two honest observations about that quadrant: (a) it's vacant partly because it's *hard* (CRDT 3D editing + A/V is exactly the hard part already built); (b) it's vacant partly because incumbents' buyers haven't demanded it — the validation doc's open question #1 (does built-in collab/A-V actually win deals?) applies here too and must be tested, not assumed.

...
> **Real-time collaborative browser 3D editing is no longer a differentiator.** It ships in *at least three* adjacent products — **PlayCanvas** (game engine), **Spline** (design), **Tabula Sono** (VTT). WorldFoundry cannot claim "Figma-for-3D, in the browser, together" as *novel* — that lane has incumbents. **Occupied ≠ closed:** you can still *choose* to compete there (Figma entered a design market full of Sketch/Adobe; Slack entered "taken" chat) — you just can't win on *"we have collaboration."* Compete on the differentiators, treat collaboration as table-stakes.
...
**PlayCanvas — the direct analog, and it already ships (high confidence).** *"Real-time collaboration is at the heart of the PlayCanvas Editor… Multiple users can work together to build a scene"* — Figma-style presence bar, per-user colors, live selection, colored camera frustums in the viewport. Snap-owned (acquired 2017, reported 2018); **Free $0 / Personal $15/mo / Org $50/seat/mo**, with real-time collaboration on *even the free tier* (paid gates privacy, not collab); used by King, Disney, Nickelodeon. **Verdict: the generic "browser + collab + game engine" slot is *occupied* — so "we have collaborative browser 3D editing" is not a *novel* claim.** But occupied is not off-limits: WorldFoundry can *choose* to compete head-to-head; it just can't win on collaboration-as-differentiator. And PlayCanvas has real soft spots to attack if we do — **Snap-owned** (users bet their tool on an ad company's roadmap), a **proprietary cloud editor** (only the *runtime engine* is open-source; the editor and hosting are closed and cloud-locked), **GPU-required**, and **subscription-priced** — against which WF could differentiate on openness, self-hosting, buy-once pricing, no-GPU/no-FPU reach, and retro export. What PlayCanvas is *not*: a fixed-point/tiny/embedded engine, and it has no retro-ROM export or VTT features (an absence inference from its WebGL/WebGPU float architecture — technically sound; the [embedded-roadmap probe](2026-07-07-playcanvas-snap-embedded-roadmap.md) confirmed no sign of it moving there).

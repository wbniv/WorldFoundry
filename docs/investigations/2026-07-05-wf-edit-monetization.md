...
**Update 2026-07-07 (positioning pivot — read this):** a week of verified research reshaped the *pitch*, not the idea catalog. Both big-money A-verticals came back no-go (AEC + film previz — incumbent-filled), and — more fundamentally — **real-time collaboration is no longer the moat.** It's *commoditized* ([PlayCanvas](2026-07-07-competitive-landscape.md), Spline, Tabula Sono all ship browser 3D co-editing) and *weakly demanded* for structured building (pros use version control; most VTT GMs prep solo — [demand pass](2026-07-07-game-worlds-vtt-demand.md)), while built-in A/V is a neutralized non-factor ([A/V pass](2026-07-07-av-and-collab-validation.md)). The defensible moat is the **engine** — a fixed-point, deterministic runtime that runs with **no GPU and no FPU**, down to ESP32-class hardware, which no competitor can follow without a rewrite and which PlayCanvas/Snap show [no sign of chasing](2026-07-07-playcanvas-snap-embedded-roadmap.md). The idea catalog below stands; the §1 one-liner is updated to lead with the engine, not the collaborative editor.
...
The honest one-liner *(revised 2026-07-07 — see the positioning-pivot note up top)*: **"A tiny, deterministic game/world engine that runs everywhere — from the browser down to a ~$5 no-GPU/no-FPU microcontroller — with collaborative editing and one-click retro export."** The *original* pitch led with "Figma-style multiplayer + voice/video/chat"; research retired that — collaboration is commoditized and A/V is a neutralized non-factor. The engine is the moat; the collaborative browser editor is the *packaging*.

Two structural facts drive everything below:

1. **The reusable asset is the *engine*, not the collaboration** *(corrected 2026-07-07)*. The original claim here — "the reusable asset is the CRDT + WebRTC collaboration substrate" — was retired: real-time collaboration is commoditized (PlayCanvas/Spline/Tabula Sono), so the CRDT doc + WebRTC A/V + wasm viewport are *packaging*, and the only un-copyable asset is the fixed-point / no-GPU / no-FPU engine. **Separately, the *format* is the gate to Part A:** today the editor edits `.lev`/OAD only, so every non-game industry requires a **general scene-format layer** — at minimum [glTF](https://www.khronos.org/gltf/) (*GL Transmission Format*, the Khronos-standard interchange format for 3D scenes, often called "the JPEG of 3D") import/export, plus [USD](https://openusd.org/) (*Universal Scene Description*, Pixar's film-pipeline scene format) for film work — plus auth, hosted persistence, TURN/SFU relay infrastructure (TURN — the relay servers WebRTC traffic falls back to when peers can't connect directly; SFU — a selective forwarding unit that routes multi-party video), and billing before the first dollar arrives. The Part B industries mostly do not.

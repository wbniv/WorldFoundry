...
What the Android plan never did: **device verification** (Phase 3 Step 7 is ⬜). Neither
a phone nor a TV has ever run the APK. Additionally:
- No TV **banner image** exists — Google TV launcher shows a blank tile without
  `android:banner` on `<application>`.
- **Audio is silent stubs** — acceptable for v1; [audio-assets-from-iff](../plans/2026-04-18-audio-assets-from-iff.md) is the
  follow-up.
- **No CI for Android** — only iOS has a Codemagic workflow; Android builds are
  local-only.
...
1. Add `android-apk-debug` workflow to `codemagic.yaml` (after the two existing iOS
   ...
   - **Build step:** `./gradlew :app:assembleDebug`
...
Android-specific audio code is needed. The only required change is for
`audio-assets-from-iff` to land, removing the loose `.mid`/`.sf2` file dependency that
currently means audio init silently skips on devices without those files.
...
- 4K framebuffer optimisation (EGL surface from `ANativeWindow` already adapts to the
  display size — no extra code)

...
- Symptom + workaround → [`docs/level-design-troubleshooting.md`](../../WorldFoundry.2026-new-level/docs/level-design-troubleshooting.md).
- New convention/technique → [`docs/level-building.md`](../../WorldFoundry.2026-new-level/docs/level-building.md).
...
The spawned coins ARE the collectibles, so `coin_template` becomes a new `Gold` actor class backing the existing [`gold.oas`](../../WorldFoundry.2026-new-level/wfsource/source/oas/gold.oas) stub (today `TYPEHEADER(Gold,Gold)` with **no backing C++ class**).

**Wiring (verified):**
- **Enum:** add `Gold_KIND` to [`wfsource/source/oas/objects.es`](../../WorldFoundry.2026-new-level/wfsource/source/oas/objects.es) (the codegen source; `objects.h`/`objects.e` are generated — `objects.e` header says "created from object.es, DO NOT MODIFY"). Append after `Alias_KIND` so existing kind values don't renumber.
- **Factory:** wire the `Gold` case into the OAS object factory ([`objects.c`](../../WorldFoundry.2026-new-level/wfsource/source/oas/objects.c) switch on `EActorKind`, mirroring `Generator_KIND`/`Spike_KIND`).

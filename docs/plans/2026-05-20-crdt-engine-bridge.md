...
| D6 | Doc field name → `wfmut` path | **Name→path table in the bridge** (`"Mass"`→`"movebloc.Mass"`, …) covering the 15 `kPropMap` fields, since `PropField`/`OadEntry` ([oad_reader.h](../../engine/wf_edit/oad_reader.h)) carry no block. Transform names (`Position`/`Orientation`) route to the transform calls, not a path. | `kPropMap` keys are the only block info; mirroring them in a tiny editor-side table is the least-coupling way. (When D5's codegen lands, both the engine `kPropMap` and this table generate from one schema source.) |
...
### 1. Identity mapping — `content[i]` ↔ `wfmut::ActorIdx` (the one real unknown) — first
...
### 2. Field translation — Doc field → `wfmut` op + coerced value
...
### 3. Wire commit → bridge → `wfmut` → viewport
...
### 4. Docs + status sync
- Plan `**Status:**` → Done w/ actuals (hours, [feedback_plan_duration_tracking](/home/will/.claude/projects/-home-will-WorldFoundry/memory/feedback_plan_duration_tracking.md)); prepend a one-sentence [wf-status.md](../../wf-status.md) Summary paragraph + Active row ([feedback_wf_status_paragraph_length](/home/will/.claude/projects/-home-will-WorldFoundry/memory/feedback_wf_status_paragraph_length.md)); tick the design-doc bridge milestone; log the `kPropMap`-codegen follow-up (D5) and the `observe_deep` remote-edit follow-up (D3) in [TODO.md](../../TODO.md) ([feedback_log_discoveries_in_todo](/home/will/.claude/projects/-home-will-WorldFoundry/memory/feedback_log_discoveries_in_todo.md)).

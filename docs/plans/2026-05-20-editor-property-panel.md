...
> **Scope update (2026-05-20, per Will):** the `.lev` source is **self-describing** — the Doc carries each field's **name** (`{ 'I32' { 'NAME' "Mass" } … }`), value, **enum current-label** ("Model Type: 11 Mesh"), and the OBJ's **class** ("Class Name: statplat"). So **Phases 1–2 (read-only display) need no OAD reader** — names/values/labels come straight from the Doc. **OAD reading (D1/D1a/D2) returns in scope at Phase 3, the properties *editor*** — editing safely needs the OAD's full enum *option lists* (dropdowns), min/max (sliders), and precise `(ButtonType×showAs)` dispatch, which the `.lev` doesn't carry. The **only** thing that stays out of scope is **running the editor from a compiled `cd.iff`/level binary** (not self-describing). Phase 2 does read-only widget dispatch from `chunk_type` + field-name + inline enum-labels (`.lev`-only).
...
- **Gate:** screenshot of a richer read-only panel on a snowgoons actor (CamShot/Director with colour + mailbox + enum fields). Precise `showAs` dispatch (full enum dropdown lists, sliders w/ min-max, exact COLOR/CHECKBOX disambiguation) lands with the OAD in Phase 3.

### 3. Properties **editor** — editable widgets → Doc (**OAD reading returns here**)
- **Bring in the OAD reader (D1/D1a/D2):** link our C++ `QObjectAttributeData`/`iff` readers into `wf_edit` **gated `WF_ENABLE_EDITOR`** (never in the runtime engine — verify `wf_game`/NDK builds byte-unchanged, D1a). Resolve each OBJ → its class's `.oad` — the class is in the Doc already (`Class Name` = e.g. `statplat`), so resolution is just class-name → `.oad` lookup.
- Use the OAD per field for **safe editing**: `(ButtonType×showAs)` → the right editable widget; full enum **option lists** → dropdowns; min/max → sliders/clamps; `EULR` in revolutions; mailbox combo from `mailbox.inc` (D7).
- `provider`/`setter` (D5): each widget reads/writes its CRDT leaf; edits commit to the Doc in a transaction. Re-reading the Doc reflects the edit. (Viewport does **not** update yet — that's the bridge.)
- **Gate:** edit an int / string / vec3 / enum (from its OAD dropdown) → value persists in the Doc (read back, and via `levtree print` round-trip if wired). ASan over OAD-read + edit→commit→read. Runtime build byte-unchanged. Screenshot.

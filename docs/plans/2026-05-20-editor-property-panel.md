...
### 2. OAD reader + OAD-driven widget dispatch (read-only render)
- **Bring in the OAD reader (D1/D1a/D2):** link our C++ `QObjectAttributeData`/`iff` readers into `wf_edit` **gated `WF_ENABLE_EDITOR`** (never in the runtime engine — verify `wf_game`/NDK builds byte-unchanged, D1a). Resolve each OBJ → its class's `.oad`: the class is in the Doc (`Class Name` = e.g. `statplat`), so it's a class-name → `.oad`-path lookup (find where the level's `.oad`s live).
- `widget_for(button_type, show_as, …)` from the design-doc table (D3), correlated with the Doc's positional field chunks: int/float, `VEC3`→3 floats, `EULR`→3 angles in revolutions ([feedback_angles_in_revolutions](/home/will/.claude/projects/-home-will-WorldFoundry/memory/feedback_angles_in_revolutions.md)), `BOX3`, single- vs multi-line string, enum (current label + full **option list** from OAD; ≤4 row / 5+ grid), checkbox, colour swatch, mailbox name, file/mesh, object-ref (name + ⚠ if missing), sections/groups. Strip `(S.W.F)` suffixes. Fallback chain `showAs → chunk_type → raw text` (D6).
- **Gate:** screenshot of a rich read-only panel on a snowgoons actor (CamShot/Director: COLOR + MAILBOX + DROPMENU fields render as swatch/combo/enum) matching the widget gallery. Field/widget choices spot-checked vs `panels.py`. Runtime build byte-unchanged; ASan over OAD-read→correlate→render.

### 3. Properties **editor** — editable widgets → Doc

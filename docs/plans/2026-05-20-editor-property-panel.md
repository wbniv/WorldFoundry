...
- **Correlation = sequence-alignment, not index-zip or name-lookup (D-corr).** Verified against House: the `.lev` field-name order aligns with `<class>.oad`'s data-entry order in long contiguous runs, modulo a small base prefix (`Position`/`Orientation`/`Global Bounding Box`/`Class Name`, absent from `statplat.oad`) and a minor `Script`/`Notes`/`slopes` reorder — and field names **duplicate** (`Mesh Name`/`Model Type` appear twice), so name-lookup is ambiguous. Align the two ordered name-sequences (anchor on matching runs, `difflib`-style), absorbing prefix/dups/reorders; matched `.lev` fields take `showAs`/`ButtonType`/`Min`/`Max` from the OAD entry, unmatched fall back to `chunk_type`.

...
- `docs/plans/2026-04-15-lua-engine-fixes.md` (cosmetic: add cross-reference noting this plan defines the convention)
- one of: `docs/scripting-languages.md` (preferred) or `docs/plans/README.md` (new) — host for the unified status table.

Reference (read-only while editing):
- `wftools/wf_viewer/stubs/scripting_stub.cc` — canonical `ScriptRouter` + `lua_engine` namespace shape.
- `wftools/wf_viewer/stubs/scripting_js.hp`, `scripting_wasm3.hp` — current ABI for already-landed engines.
...
6. No code changes in `wftools/` or `wfsource/` — this is a docs-only pass.

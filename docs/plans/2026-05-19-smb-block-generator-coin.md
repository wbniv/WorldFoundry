...
**Status:** Spawn crash fixed 2026-05-21 — `tests/repro_gold_spawn_crash.py` passes (two clean `AddObject ok`, no `terminate`, no ASan abort). Bidirectional dispatch (`actor.cc:1805`) + `Gold::Collision()` pickup logic verified by code review; Gold↔Player interactive pickup needs user play-test at full frame rate (bridge test is unreliable at debug-build 1 FPS). Coin SFX and interactive pickup test remain; everything else done. Supersedes [`2026-05-19-smb-coin-fix-and-template.md`](2026-05-19-smb-coin-fix-and-template.md) (player-driven Phase 2, committed in `0adf1d4`). All three engine changes landed (bidirectional collision dispatch `actor.cc:1805`; generator-runs-script `4d4dff8`; `Gold` collectible class `7bf3de0`) plus the level rework (qblocks = `generator` class with the 3-state self-detect script + `coin_template` as a `gold`). **Block→gold spawn crash root-caused 2026-05-21** — *not* the Jolt position desync the step-0 plan blamed, but a **stale `gold.oad`**: it was generated Apr 28 *before* `Gold_KIND` existed in `objects.lc`, so the `MovementClass` default baked as `0` (`NULL_KIND`) instead of `26` (`Gold_KIND`), tripping `Gold::kind()`'s `assert(MovementClass == Gold_KIND)` in `ConstructTemplateObject`. Plus `objects.{col,car,ctb}` (collision tables) were never regenerated for Gold → a second OOB in `Actor::CanCollide`. Fixed by regenerating `gold.oad` + `objects.{car,col,ctb}` from the masters (`objects.mac` now carries the Gold `COLTABLEENTRY` rows) and rebuilding. See [docs/level-building.md § Creating a new OAD class](../level-building.md#creating-a-new-oad-actor-class). Only the coin pickup **SFX** is deferred.

...
**Date:** 2026-05-21
**Status:** **M1–M2 done 2026-05-21 — actor add/delete + save; M3 (docs) pending.** Scope approved by Will: ship layers 1 + the Outliner UI, handle the crux the safe way (suspend field→viewport propagation after a structural edit). The consumer the [lossless Doc schema](2026-05-21-lossless-doc-schema.md) unblocked. **M1** ✓: `wfcrdt::Array::remove` (wraps yffi `yarray_remove_range`, bounds-guarded); `level_doc` `DeleteActor`/`DuplicateActor` (clone via `DocChunkToInput`); headless `WF_EDIT_STRUCT_TEST` proof — snowgoons 36 → `dup=0` → **37** (re-parses; House count 2 = faithful clone) → `del=3` → **35**, ASan-clean. **M2** ✓: Outliner Duplicate/Delete buttons + Del key; a structural edit sets `structural_dirty` → field→viewport propagation suspended (D3 guard) with a toast; [screenshot](../../tests/screenshots/wfedit_outliner_struct.png) (37 actors, the duplicated House selected). Save persists the new count (lossless schema). The `wfcrdt::Array::remove` round-trip is covered end-to-end by the structural-save proof (a dedicated wrapper unit test was skipped as redundant). Next: **M3** — docs + the live-sync / stable-id follow-ups.
...
### 3. Docs + status sync
- Plan Status → Done w/ actuals; [wf-status.md](../../wf-status.md) row; design-doc note; log the **live-structural-sync** follow-up (D4) + the **stable-id identity map** (D3 proper fix) in [TODO.md](../../TODO.md).

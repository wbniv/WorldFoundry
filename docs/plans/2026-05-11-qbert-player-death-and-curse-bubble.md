...
**Schema fix — critical lesson:** the curse bubble must use `STATPLAT_OAD` (cube schema), **not** `ENEMY_OAD`. Different OAS classes carry different field defaults; the bubble's `wf_*` Blender properties only override fields the chosen OAS exposes — everything else falls back to the class default. ENEMY_OAD's defaults expect the Enemy lifecycle (spawn / active / despawn phase wiring); a script-less Enemy actor can sit in "not-yet-active" state where rendering is suppressed even though `Mass=0` and `Visibility Mailbox=1` look correct. With ENEMY_OAD the actor loaded (object count 59) but didn't appear in the render set — `grep -c RenderActor3DAnimates wf_game.log` stuck at 41 instead of 42. Switching to STATPLAT_OAD bumped it to 42 and the white sphere appeared above the apex on the next build.

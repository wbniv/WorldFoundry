...
**Status:** Phase 2D step-1 verified — snowgoons textures on iOS (2026-04-22). `PixelMap::GetRoot()` + `GetRootPixelBuffer()` inline accessors expose the raw RGBA buffer from iOS-side backend_metal.mm; a `std::unordered_map<const PixelMap*, id<MTLTexture>>` caches per-root-PixelMap MTLTextures built via `MTLPixelFormatRGBA8Unorm` + `replaceRegion:`. `MTLSamplerState` is linear/linear + repeat/repeat, matching gfx/pixelmap.cc's GL setup under GFX_ZBUFFER. 1×1 white fallback MTLTexture keeps `[[texture(0)]]` always bound (untextured draws have use_tex=0 so the sample is ignored). MSL fragment shader now samples when use_tex != 0, modulating color × texel. Sim-verify confirms `MetalBackend uploaded texture #0 (256x256)` + `#1 (256x256)` and the screenshot shows **snow ground + green hedges + bare tree branches + teal/purple accents** — real snowgoons scene, not placeholder geometry. Still only ~12 tris/frame emitted (camera frame shows a narrow slice), and house/snowgoons/player aren't in the current camera view — but texturing is correct. Phase 2D step 2+: camera/frustum investigation for full-scene rendering; step 3: lockstep render parity vs. Linux.

...
**Status:** Phase 2C-A verified (2026-04-22) — engine main loop boots on iOS for the first time. Spawned `HALStart` on a pthread from `WFRootViewController viewDidLoad`; snowgoons `cd.iff` loads end-to-end (37 objects, 37 textures compile through PixelMap, actors instantiate, zforth bootstraps the camshot module); game loop runs silently emitting normal `BungeeCameraHandler::GetWatchObject` chatter. Seven bring-up commits past Phase 2B3: GL-stub TU for `pixelmap.cc`'s 7 GLES symbols (no OpenGLES.framework link on iOS), stdio `dup2` to `NSTemporaryDirectory()/wf.log` + signal trap (Android-parallel), HAL-boundary `fprintf(stderr)` tracers, `ma_engine_config::noDevice=MA_TRUE` to unstick the CoreAudio RPC deadlock on headless Mac mini, `Display::PageFlip` timeval-delta fix (was feeding absolute Unix epoch into int16 seconds slot), Codemagic webhook wired via GitHub App + manual webhook register (pushes now auto-trigger builds), `docs/codemagic-setup.md` captures the setup. Sim-verify still cornflower blue because Phase 2C-B hasn't wired `SetCurrentEncoder` yet — Metal backend is alive but nothing drives it per-frame. Phase 2C-B next: CADisplayLink on main thread hands the backend a live `MTLRenderCommandEncoder`; `Display::PageFlip` blocks on a semaphore instead of sleeping 16ms. Phase 2D: first snowgoons triangle on-screen.

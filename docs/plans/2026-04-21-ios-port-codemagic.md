...
**Status:** Phase 2B2 verified (2026-04-22) — full engine (~120 sources: pigsys, streams, cpplib, math, memory, iff, mailbox, hal/ios, gfx core, input, asset, baseobject, movement, room, physics, anim, timer, scripting, game, ini, loadfile, profile, renderassets, savegame/linux, menu, particle, audio/linux) compiles and links for arm64 iOS Simulator; `wf_game.app` runs on iPhone 17 Pro Sim with `MetalView init, device=Apple iOS simulator GPU` and `cd.iff opened, size=174080` in the unified log. Three bring-up commits past Phase 2A: compile `audio/linux/*.cc` as Obj-C++ (miniaudio on Apple unconditionally `#include <AVFoundation/AVFoundation.h>` — documented integration pattern); add AVFoundation/AudioToolbox/CoreAudio frameworks; provide iOS-only no-op stubs for the 8 GL-backend `RenderObject3D::RenderPoly3D*` methods in `engine/stubs/renderpoly3d_stubs.cc` (Metal backend replaces them in Phase 2B3). Phase 2B3 next: real `RendererBackend` subclass in `backend_metal.mm` + MSL port of `backend_modern.cc`'s phong+fog shaders + wire `backend_factory.cc` iOS branch, then Phase 2C wires engine frame loop to MetalView's `CADisplayLink`, Phase 2D gets the first snowgoons triangle.

...
**Status:** Phase 2B3 verified (2026-04-22) — `hal/ios/backend_metal.mm` implements `RendererBackend` method-for-method vs `backend_modern.cc`; MSL vertex + fragment shaders inline, runtime-compiled via `[MTLDevice newLibraryWithSource:]`. `backend_factory.cc` picks Metal on iOS via `WF_TARGET_IOS` ifdef, pulled into CMake as a one-off since rest of `gfx/glpipeline/` is GL-only. Single BGRA8Unorm pipeline state, per-vertex float3 pos/color/normal + float2 uv. Build + link clean on Codemagic, sim-verify still shows cornflower blue (lazy init hasn't run — nothing drives the backend yet). Phase 2C next: wire MetalView's `CADisplayLink` tick → `SetCurrentEncoder(...)` → engine frame loop → `EndFrame` → present/commit; this will boot the engine main loop for the first time on iOS and likely surface init-order / missing-platform-service failures that need fixes. Phase 2D gets the first snowgoons triangle on-screen.

...
**Status:** Phase 2C-B verified (2026-04-22) — WF's geometry renders through Metal on iOS for the first time. Engine thread now owns the frame: `WFMetalView` is purely a layer host (CADisplayLink removed), `backend_metal.mm` exposes `WFIosRenderBegin(r,g,b)` / `WFIosRenderEnd()` extern-C bridge functions, `Display::RenderBegin/RenderEnd` in `display_ios.cc` call them around the per-frame draw calls. Five commits past Phase 2C-A: engine-thread-owns-frame rewrite, `QuartzCore/CAMetalLayer.h` import, one-shot fprintf tracers (diagnosed `cpu=0 tris, encoder=live`), replace the Phase 2B2 `renderpoly3d_stubs.cc` no-ops with the real `gfx/glpipeline/rend{f,g}{c,t}{l,p}.cc` impls (backend-agnostic, just decode Primitive → RBVertex → `DrawTriangle`). Sim-verify shows `MetalBackend ready (device=Apple iOS simulator GPU)` in wf.log, `first MetalBackend EndFrame — cpu=12 tris, encoder=live`, and actual scene geometry on-screen — untextured, portrait-cropped (iPhone orientation vs. landscape projection mismatch). Phase 2D: wire texture upload (PixelMap → MTLTexture), fix portrait/landscape orientation handling, validate snowgoons renders recognisably vs. Linux.

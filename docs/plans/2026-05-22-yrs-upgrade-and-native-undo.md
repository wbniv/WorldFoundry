...
> **Phase B result:** native [`UndoManager`](https://docs.yjs.dev/api/undo-manager) wrapped in `wfcrdt.hpp`/`.cpp` over the `yundo_manager_*` C ABI (move-only RAII; 500 ms coalescing default). **Local-only undo** realized with zero per-edit bookkeeping: `Doc::begin()` now tags every local edit `kOriginLocal` and the manager `add_origin`s it, while remote applies switched to a new `Doc::beginRemote()` (`kOriginRemote`, never captured) at the two SYNC sites (`CollabDrain` + initial full-state). Editor wiring (`main.cc`): `EditorCtx::undo` → a `main()`-owned manager constructed right after Doc population with `addScope("content")`; `DoUndo`/`DoRedo` call undo/redo then a shared `ReSyncAfterDocChange` (extracted from `CollabDrain`) + a toast; Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y keybindings (gated `!typing`, `!Shift` on plain-Z) + a greying **Edit** menu. Verified: `wfcrdt_wrapper_test` 13/13 (incl. 3 new native-undo cases — basic undo/redo, local-only origin gating, and **collab local-only across a real sync round-trip**: two Docs + two `UndoManager`s, A's local edit never enters B's history, B's Ctrl+Z can't revert it, A's own undo syncs the revert back to B), `WF_EDIT_UNDO_TEST` headless harness + `wf_edit_undo` CTest pass (field/dup/del checkpoints restore), and the delete→undo screenshot below shows the Outliner repopulated + "undo" toast. The full two-instance GUI collab-undo run is parked in TODO § COLLABORATIVE EDITOR (the origin-gating property it would exercise is now covered deterministically by the wrapper test).
...
4. **Collab local-only undo:** covered deterministically by `test_undo_local_only_across_sync` in `wfcrdt_wrapper_test.cc` — two Docs + two `UndoManager`s + a real `stateDiff`/`apply` round-trip (B applies A's update via `beginRemote`): A's edit never enters B's undo stack, `ub.undo()` is a no-op on it, and A's own undo syncs the revert back to B. The faithful two-instance GUI run (relay + two `wf-edit --room` windows) is parked in TODO § COLLABORATIVE EDITOR as belt-and-suspenders confirmation of the GUI wiring.

...
### 2. Structural-edit proof (the payoff)
- Headless `WF_EDIT_STRUCT_TEST`: remove an actor from `content` (and/or append a cloned one) via the wfcrdt Array, then save → the `.lev` has the right `OBJ` count and the structural change — the thing the retained-JSON patch **could not** do. (No Outliner UI — that's a separate plan; this proves the schema supports it.)
- **Gate:** delete actor N → saved `.lev` re-parses, has N−1 OBJs, every surviving OBJ byte-identical; ASan-clean.

### 3. Docs + status sync
- Plan `**Status:**` → Done w/ actuals; [wf-status.md](../../wf-status.md) row; mark the [TODO.md](../../TODO.md) lossless-Doc-schema follow-up done; note in the [save plan](2026-05-21-editor-save-roundtrip.md) that the retained-JSON side-channel is gone; design-doc note.

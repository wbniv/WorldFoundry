...
These are the design doc's [editor + debugger convergence](../investigations/2026-05-18-collaborative-level-editor-design.md) direction — time-deferred but explicitly committed, not speculative. Building the API once, well, as a first-class engine artifact serves all of them.
...
- [Codemagic 500 min/mo budget](../../../.claude/projects/-home-will-WorldFoundry/memory/project_ios_port_blocker.md) protected.
...
| CRDT ownership | **Editor owns Y.Doc** | Engine stays Rust-free. Engine exposes plain C++ mutation API. Editor's CRDT bridge translates Yrs ops to engine calls. Serves debugger + replay + headless reuse, not just the editor. |
...
1. **Vendor y-crdt as submodule.** `git submodule add https://github.com/y-crdt/y-crdt wftools/y-crdt`. Pin to the latest tagged release (check at submodule-add time). Smoke: `cargo build --release -p yffi --manifest-path wftools/y-crdt/Cargo.toml` produces `target/release/libyffi.a` + the pre-generated `libyrs.h`.
...
5. **Smoke test executable.** `engine/crdt/wfcrdt_smoke.c` — minimal C program that exercises:
   ...
   - `ymap_insert` + `ymap_get` on a top-level Y.Map (the `meta` map per the [CRDT schema](../investigations/2026-05-18-collaborative-level-editor-design.md))
...
6. **CI / Codemagic verification.** No Codemagic changes needed — `WF_ENABLE_CRDT=OFF` by default, Codemagic's existing `cmake` invocations don't pass the flag, nothing changes. Add a comment in [codemagic.yaml](../../codemagic.yaml) noting the toggle exists but is deliberately off for shipped builds. Manual local check: `task build && ./engine/wf_game` works without Rust (rename cargo or use `PATH= cmake ...` to confirm).
...
8. **Docs.** Update [editor design doc](../investigations/2026-05-18-collaborative-level-editor-design.md) Tier 2 entry to reflect the locked-in "editor owns Y.Doc" decision and link this plan. Update [wf-status.md](../../wf-status.md) per [feedback_wf_status_rolling_summary](../../../.claude/projects/-home-will-WorldFoundry/memory/feedback_wf_status_rolling_summary.md). Add a [TODO.md](../../TODO.md) entry per [feedback_log_discoveries_in_todo](../../../.claude/projects/-home-will-WorldFoundry/memory/feedback_log_discoveries_in_todo.md) noting that `libwfcrdt.a` should NOT be linked into `libwfengine.a` unless the architectural decision is explicitly revisited.
...
- **IFF↔Y.Doc translator** — separate v1 line item (~2–3 wk). Needs [iffcomp-rs](../../wftools/iffcomp-rs/) reshaped as a Rust library (it's a CLI today). Builds on top of `libwfcrdt.a`. Wire format: round-trip `.iff.txt` ↔ Y.Doc per the [CRDT schema](../investigations/2026-05-18-collaborative-level-editor-design.md).
...
- [`CMakeLists.txt`](../../CMakeLists.txt) — root; add `WF_ENABLE_CRDT` option.
- [`Taskfile.yml`](../../Taskfile.yml) — add `build-editor` task.
- [`codemagic.yaml`](../../codemagic.yaml) — comment only (no functional change).
- [`docs/investigations/2026-05-18-collaborative-level-editor-design.md`](../investigations/2026-05-18-collaborative-level-editor-design.md) — update Tier 2 entry with the locked decision.
- [`wf-status.md`](../../wf-status.md) — add status row.
- [`TODO.md`](../../TODO.md) — note the "don't link wfcrdt into wfengine" constraint.
...
- [Editor design doc § Tier 2 Engine↔CRDT bridge](../investigations/2026-05-18-collaborative-level-editor-design.md) — the bridge architecture this binding enables.

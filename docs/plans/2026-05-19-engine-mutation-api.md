...
**Date:** 2026-05-19
**Status:** **Mostly done 2026-05-19 (~3 h vs ~1–2 wk estimate) — `SpawnActor` runtime path still open.** The read/write/field/mailbox/reload/remove surface is done and smoke-green; `SpawnActor` compiles and returns an index but the actual runtime spawn aborts (see tail). Six implementation commits + one consolidation commit on top of the wfcrdt wrapper, plus the user-feedback-driven WF_DEBUG_BRIDGE-independence refactor. `wfmut::` surface covers Set/GetActorPos + Set/GetActorOrientation + Set/GetActorField (int64/double/string) + Set/GetActorFieldString stub + ReloadActorScript + SpawnActor + RemoveActor + Set/GetMailbox + thread-local lastError. Bridge SET_TRANSFORM / SET_PROP / SET_MAILBOX cases now route through wfmut; the duplicate `kPropMap` in debug_server.cc is gone. `wf-edit` (editor build) smoke = 30/30 green (7 transform + 12 field + 3 spawn/remove + 6 mailbox + 2 cross-cutting; verified 2026-05-19, the smoke grew orientation T9–T11 + cross-cutting X1–X2 after the original 24/24 count was written); `wf_game` (lean) and `wf-edit` (editor stack) coexist as separate binaries. Happy-path spawn tests (SR1/SR2/SR7/SR8/SR10) deferred to manual / bridge-integration verification — first runtime-spawnable template in smb_w1_1 aborts on generic spawn-at-player-pos, needs a known-safe fixture. The crash (spawned template's Jolt body created at the authored parking pos `(-50,0,0)` then `terminate called without an active exception`) is tracked in [template-object-jolt-body-sync](2026-05-19-template-object-jolt-body-sync.md) — whose Option A fix is **already committed** (bundled into the RTTI commit `0adf1d4`) but **never runtime-verified**; treat `SpawnActor` as committed-but-unconfirmed until the SMB Gold work exercises it.

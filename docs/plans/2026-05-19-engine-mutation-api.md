...
| Compile-time gating | **All editor surfaces (wfmut + wfcrdt + debug bridge + REST API) collapse to ONE `WF_ENABLE_EDITOR` CMake option.** Default OFF. Force OFF on Android/iOS. No Debug-build auto-on cascade — the "original game engine" is the default for *every* build type; editors opt in explicitly via `task build-editor`. | Original plan said "fold into libwfengine.a, no opt-out" — wrong on two counts: (a) wfmut won't build on Android/iOS where the editor host doesn't exist, (b) the lean game engine is what `task build` should produce in MOST cases. User flagged mid-step-2 (2026-05-19): "bigger than wfmut — it's all the editor code — needs 1 compile-time switch — that includes the previous 2 plans you've implemented." Memory: [feedback_editor_code_compile_gate](/home/will/.claude/projects/-home-will-WorldFoundry/memory/feedback_editor_code_compile_gate.md). Implication: `WF_REST_API` / `WF_DEBUG_BRIDGE` / `WF_ENABLE_CRDT` collapse into the umbrella in the same change — these three options retire. |

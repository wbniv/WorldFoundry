...
**Date:** 2026-04-14
**Status:** **landed 2026-04-14.** `engine/vendor/lua-5.4.8/` is in tree;
...
WF's Linux build currently relies on the system `lua5.4` package (`-llua5.4` in `engine/build_game.sh`). This has two problems flagged in both the mobile-port plan and the scripting-languages doc:
...
| File | Change |
|------|--------|
| `engine/vendor/lua-5.4.8/` | **New** — upstream tarball; record SHA256 in `engine/vendor/README.md` |
| `engine/vendor/README.md` | Add Lua entry |
| `engine/build_game.sh` | Drop `-llua5.4`; drop system Lua `-I`; add compile of `engine/vendor/lua-5.4.8/src/*.c` (minus `lua.c`/`luac.c` — those are the interpreter and compiler `main`s, not library TUs); add `-DLUA_USE_POSIX -DLUA_USE_DLOPEN`; add `-ldl` |
| `wftools/engine/stubs/scripting_stub.cc` | Update `#include <lua5.4/lua.h>` → `#include <lua.h>` (three such lines) |

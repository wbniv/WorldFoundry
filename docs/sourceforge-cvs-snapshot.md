...
- **`BaseObject` root-class extraction** ([investigations/2026-05-30-baseobject-2003-extraction.md](investigations/2026-05-30-baseobject-2003-extraction.md)) — `baseobject/baseobject.hp,v` rev 1.1 dated **2003-05-15 05:29:04** by `kts` ("first added: base class which all game objects are derived from…"); same-sitting `physicalobject.hp` rev 1.11 (05:46:46) reparents `PhysicalObject` onto it. Consolidated msgport/CommonBlock/OAD down to a shared root (good) and added a speculative `BaseObject`-above-`PhysicalObject` boundary for non-physical objects that never materialised (linear chain to this day).

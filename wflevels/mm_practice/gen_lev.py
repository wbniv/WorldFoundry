...
# 5. Camera — must be inside Room01 bounds (X:-12..12, Y:-4..24, Z:-3..8)
def emit_camera():
    begin_obj("Camera")
    vec3("Position", 0.0, -2.0, 7.0)
    eulr("Orientation", 0.0, 0.0, 0.0)
...
# 9. CamShot01 — must be inside Room01 bounds (X:-12..12, Y:-4..24, Z:-3..8)
def emit_camshot():
    begin_obj("CamShot01")
    vec3("Position", 0.0, -2.0, 7.0)
    eulr("Orientation", 0.0, 0.0, 0.0)

"""
blender_mm_silly.py — Build the MM Silly level from ROM data in Blender.

Run via Blender MCP (execute_blender_code) or headless:
    blender --background --python blender_mm_silly.py

Path layout (GAME_UNIT=0.05, SEG_LEN=2.5, PATH_HALF=2.0):
    Segs 0-2: heading 72.42° (NNE); h_center=34-36 → Z=1.45-1.55m; trough walls
    Segs 3-4: goal platform (h_center=5 → Z=0)
    Path extent: X ~[-2,5], Y ~[-2,12]; walls up to ~4.35m
    Total XY span: ~8 m NNE to goal
"""
...
OUT_LEV    = os.path.join(SCRIPT_DIR, 'mm_silly.lev')

def oad(name):
    return os.path.join(OAD_DIR, f'{name}.oad')


ROOM_POS        = (-1.5,  0.7,  3.8)
ROOM_LOCAL_BBOX = (-7.0, -11.0, -10.0,  7.0, 11.0, 10.0)

SPAWN_POS    = ( 0.3,   0.3,  1.65)
CAMSHOT_POS  = (-6.0,  -8.0, 10.0)
TARGET1_POS  = ( 0.0,   0.0,  0.0)
TARGET2_POS  = ( 3.1,   9.5,  0.0)
LIGHT_POS    = (-1.5,   0.7, 12.5)
CAMERA_POS   = (-5.7,  -7.7, 11.65)

DIRECTOR_SCRIPT = (
    r'\\ wf' '\n'
    r': init-game  INDEXOF_TIME read-mailbox 20 +  2 write-mailbox'
...
_rtb_ns['build_path_mesh']('Silly', _rtb_ns['load_levels']())

print(f'[silly] Exporting to {OUT_LEV}')
try:
...
print(f'[silly] Done. Objects in scene:')

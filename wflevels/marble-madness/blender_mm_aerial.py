"""
blender_mm_aerial.py — Build the MM Aerial level from ROM data in Blender.

Run via Blender MCP (execute_blender_code) or headless:
    blender --background --python blender_mm_aerial.py

Path layout (GAME_UNIT=0.05, SEG_LEN=2.5, PATH_HALF=2.0):
    Segs 0-1: heading  4.22° (nearly East); h_center=23-24 → Z=0.90-0.95m
    Segs 2-7: heading 42.19° (NE); h_center=11-13 → Z=0.30-0.40m; trough walls
    Seg  8:   heading 52.03° (NNE); h_center=10 → Z=0.25m
    Seg  9:   error (out of range) — skipped by decoder
    Segs 10-11: goal platform (h_center=5 → Z=0)
    Path extent: X ~[-2,23], Y ~[-2,18]; walls up to ~3.25m
    Total XY span: ~5 m E then ~14 m NE to goal
"""
...
OUT_LEV    = os.path.join(SCRIPT_DIR, 'mm_aerial.lev')

def oad(name):
    return os.path.join(OAD_DIR, f'{name}.oad')


ROOM_POS        = (7.5,   4.0,  3.0)
ROOM_LOCAL_BBOX = (-16.0, -14.5, -9.5,  16.0, 14.5, 9.5)

SPAWN_POS    = ( 0.3,   0.3,  1.10)
CAMSHOT_POS  = (-6.0,  -8.0, 10.0)
TARGET1_POS  = ( 0.0,   0.0,  0.0)
TARGET2_POS  = (19.2,  14.4,  0.0)
LIGHT_POS    = ( 7.5,   4.0, 11.5)
CAMERA_POS   = (-5.7,  -7.7, 11.10)

DIRECTOR_SCRIPT = (
    r'\\ wf' '\n'
    r': init-game  INDEXOF_TIME read-mailbox 35 +  2 write-mailbox'
...
_rtb_ns['build_path_mesh']('Aerial', _rtb_ns['load_levels']())

print(f'[aerial] Exporting to {OUT_LEV}')
try:
...
print(f'[aerial] Done. Objects in scene:')

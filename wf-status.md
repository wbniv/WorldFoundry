...
- **Host-GL e2e harness + UnloadLevel LIFO chain fix (2026-05-18)** — End-to-end follow-up to Phase 0b sub-tasks #1/#2. Found and fixed 6 dormant LIFO violations in `~Level::~Level` / `~WFGame` / member-dtor ordering, 1 `Array<T>` allocator-misuse bug (placement-array-new vs MEMORY_DELETE_ARRAY mismatch on trivial types), 1 timing bug (HALCloseWindow before WFGame dtor segfaulted Display::~Display), and 1 Jolt ODR violation across TUs from inconsistent NDEBUG — `JPH_ENABLE_ASSERTS` was on in Jolt.a but off in wfengine, ConstraintManager's only ctor disappears when asserts are off. Added a new `wf_host_gl_e2e_test` external binary linking `libwfengine.a` with its own `main()` (proves the Phase 0a split is structurally sound) + `--cycles=N` CLI on `wf_game` + 4 CTest entries (`task test-cycle`). Took ~5 h vs the ~half-a-day estimate — latent bug chain was deeper than anticipated. See [plan](docs/plans/2026-05-18-host-gl-e2e-harness-and-unload-fix.md), [root-cause investigation](docs/investigations/2026-05-18-unloadlevel-lifo-bug.md), [docs/BUGS.md](docs/BUGS.md).
...
|------|------|--------|---------|
| 2026-05-18 | [Plan: Yrs C ABI binding](docs/plans/2026-05-18-yrs-c-abi-binding.md) | **Not started** | v1 collaborative-editor sub-task #1. Vendor [y-crdt](https://github.com/y-crdt/y-crdt) as submodule, [Corrosion](https://github.com/corrosion-rs/corrosion) for CMake↔Cargo glue, build `libwfcrdt.a` behind a default-OFF `WF_ENABLE_CRDT` option, smoke test exercises Doc/Map/Array round-trip + Yjs-wire-compat state-diff apply + observer registration. Engine stays Rust-free; editor owns the Y.Doc; engine mutation API (`SetActorField`/`SpawnActor`/etc.) becomes a first-class deliverable serving editor + DAP debugger + replay UI + headless test harness. ~1 wk estimate. |
...
| Date | Plan | Status | Summary |
|------|------|--------|---------|
| 2026-05-18 | [Plan: Host-GL e2e harness + UnloadLevel LIFO fix](docs/plans/2026-05-18-host-gl-e2e-harness-and-unload-fix.md) | **Done 2026-05-18 (~5 h)** | Phase 0b sub-task #3. Surfaced and fixed 6 dormant LIFO violations in `~Level::~Level` / `~WFGame` member-dtor ordering (commit `254c1d4`), 1 `Array<T>` allocator-misuse bug from placement-array-new vs `MEMORY_DELETE_ARRAY` mismatch on trivial T, 1 timing bug (HALCloseWindow before WFGame dtor segfaulted Display::~Display), and 1 Jolt ODR violation across TUs from inconsistent NDEBUG state — `JPH_ENABLE_ASSERTS` was on in Jolt.a but off in wfengine, and `ConstraintManager`'s only ctor disappears when asserts are off (commit `7a9ff31`). Split `hal/linux/platform.cc` into `platform_init.cc` (engine library) + `platform_main.cc` (executable shell) (commit `3094acf`); added `--cycles=N` CLI on `wf_game` for multi-cycle Load/Unload coverage (commit `f174840`); shipped a `wf_host_gl_e2e_test` external binary linking `libwfengine.a` with its own `main()` against an X11/GLX context the engine adopts via `SetHostGLContext` (commit `622fd81`); registered 4 CTest entries + `task test-cycle` Taskfile target (commit `e5e3975`). Original ~half-day estimate was off by ~10× — recalibrate. Full investigation in [docs/investigations/2026-05-18-unloadlevel-lifo-bug.md](docs/investigations/2026-05-18-unloadlevel-lifo-bug.md); dormant-bug catalogue in [docs/BUGS.md](docs/BUGS.md). |

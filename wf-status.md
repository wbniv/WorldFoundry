...
|------|------|--------|---------|
| 2026-05-26 | [Plan: Internet (WAN) voice + video via WebRTC](docs/plans/2026-05-26-internet-voice-video-webrtc.md) | **Planned (not started)** | Take `wf-edit` voice/video over the internet via real WebRTC (libdatachannel) — encrypted DTLS-SRTP media, SDP/ICE signaling over a public `wss://` relay (new `CH_SIGNAL`), public STUN with deferred coturn/Cloudflare Calls TURN, staying P2P so server cost holds ~flat at scale; see [investigation](docs/investigations/2026-05-26-internet-voice-video-nat-traversal.md). |

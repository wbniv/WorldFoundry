...
- **`wf-edit` in-editor cd.iff level picker (2026-05-25)** — Launching the editor on a bare multi-level `cd.iff` (no `:tag`) now pops a startup modal listing the archive's levels (tag/size/offset) to choose which one to load this session, rather than erroring. See [plan](docs/plans/2026-05-25-wf-edit-cdiff-level-picker.md).
...
|------|------|--------|---------|
| 2026-05-25 | [Plan: wf-edit in-editor cd.iff level picker](docs/plans/2026-05-25-wf-edit-cdiff-level-picker.md) | **DONE 2026-05-25 (~1 h)** | Launching `wf-edit` on a bare multi-level `cd.iff` (no `:tag`) shows a startup modal listing the archive's levels (`#`/tag/size/offset); choosing one loads it via the existing `cd.iff:<tag>` path — a startup pick, not runtime switching (no bridge/engine reset). Verified by tag (`--pick-level=L4`) and index (`=1`); explicit selector / bare `.iff` / text `.lev` skip the picker. Engine viewport tracking the picked level stays deferred with the level-switch work. |
...
| 2026-05-25 | [Plan: SMB pipe warp → underground coin room](docs/plans/2026-05-25-smb-pipe-warp-coin-room.md) | **Phase A DONE 2026-05-25** | Phase A proves WF's first cross-room transition: Down on a surface pipe (ActBox + player-script Down gate) warps Mario into a genuine second `room` (underground coin room), which renders lit with the camera following; five gotchas fixed and written to the designer guide (player `Moves Between Rooms`, per-room light, mutually-adjacent rooms so the camera doesn't freeze, `INDEXOF_CAMSHOT`=1921-not-1021, ActBoxOR fires via C++ overlap). Phase B (coins + exit `Warp`) remaining. |

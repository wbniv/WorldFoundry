...
- **Internet voice+video in `wf-edit` — WebRTC P2P (2026-05-26)** — Replaced the LAN-only raw-UDP Opus/VP8 transport with libdatachannel (v0.21.2) WebRTC P2P: ICE+STUN selects the best path, DTLS-SRTP encrypts all media end-to-end (even through TURN), and trickle-ICE SDP/candidates are carried over a new `CH_SIGNAL=0x05` relay channel; `ws_client` gained full `wss://` TLS support (OpenSSL) so the signaling relay is also encrypted; VoiceChat/VideoChat now expose `SetSendCallback`/`OnRemoteOpus`/`OnRemoteVP8Frame` — all raw socket code removed; libdatachannel pulled via CMake FetchContent. Phases 3 (TURN) + 4 (quick-tunnel UX) deferred. See [plan](docs/plans/2026-05-26-internet-voice-video-webrtc.md).
- **SMB state-aware power-up block + Starman wall-reversal (2026-05-26)** — The mushroom and fire flower became ONE self-determining `powerup_template` (a Generator's thrown object is fixed at load, so the *item* decides): it reads `SMB_MARIO_STATE` live and becomes a red sliding mushroom when Mario is Small or an orange stationary flower when Super+, raising the matching pickup signal — so every power-up block faithfully dispenses the right item for your tier; and the bouncing Star now reverses horizontally off the flagpole/walls by negating `XSPEED` on a side-contact normal. Pure level-Forth, no engine rebuild; bridge-verified with screenshots. See [plan](docs/plans/2026-05-26-smb-powerup-block-and-star-reversal.md).
- **SMB Fire Flower + Star power-ups (2026-05-26)** — A Fire Flower (`gold`-class collectible) raises Mario Small/Super→Fire with a scale-aware power-down chain (Fire→Super keeps the big size, Super→Small shrinks), and a bouncing Starman grants a ~10 s invincibility window that defeats enemies on touch and ignores their hits — the Star's ground-aware bounce re-launches off the real Jolt floor-contact normal (no engine restitution needed, since WF "physics" actors are kinematic CharacterVirtuals), all bridge-verified with screenshots. See [plan](docs/plans/2026-05-26-smb-fire-flower-and-star.md).
- **SMB breakable bricks (2026-05-26)** — Super Mario shatters a brick from below into a 4-fragment debris burst that despawns without scoring while Small Mario only bumps it, laid out as a faithful W1-1 brick row interleaved with the `?`-blocks plus a hidden-mushroom brick, all composition + Forth on the generator pattern. See [plan](docs/plans/2026-05-26-breakable-bricks-smb-world-1-1.md).
- **SMB Super Mushroom + power-up state machine (2026-05-26)** — A new mushroom block throws one mushroom (a `gold` actor "worth 0" + a pickup script, so no new class) that slides out on real physics; touching it flips Mario Small→Super (visibly scaled up from his feet) and a hit now powers him down to Small instead of costing a life, all bridge-verified end to end. See [plan](docs/plans/2026-05-26-smb-super-mushroom-powerup.md).
- **`wf-edit` Notes leaf renders as Markdown (2026-05-26, dormant)** — The property panel's Notes (`MultilineStr`) leaf now renders bold/headings/lists/links via `ImGui::Markdown` when idle and falls back to the edit box on click, but it stays dormant until the OAD emits the Notes field as `SHOW_AS_TEXTEDITOR` (it currently resolves to `BUTTON_XDATA`+`SHOW_AS_N_A` → `Skip`, so Notes isn't drawn yet). See [plan](docs/plans/2026-05-26-wf-edit-notes-markdown.md).
- **`wf-edit` File→Open browser + Save-As-`.lev` labeling (2026-05-26)** — File→"Open Level…" / `Ctrl+O` opens a hand-rolled ImGui browser that re-execs the editor into the picked `.iff`/`.lev`/`.lvl` (a bare `cd.iff` re-shows the picker), and a binary-loaded session now labels Save as "Save As .lev"; the planned friendly-name column was dropped because the tags are already legible slot labels and a level's identity is structural, not recoverable by positionally parsing `cd_*.iff.txt`. See [plan](docs/plans/2026-05-25-wf-edit-finish-the-level-loading-ux-ordered.md).
...
|------|------|--------|---------|
| 2026-05-26 | [Plan: Fire Mario fireball — pooled teleported generator](docs/plans/2026-05-26-fire-mario-fireball-pooled-generator.md) | **Not started** | WF's first runtime-positioned spawn, with **zero engine code**: a hidden, non-solid (meshless → no Jolt body) pool `Generator` self-parks on Mario's published spawn point each tick and fires a `Missile` fireball when he pulses its global activation mailbox; two generators (left/right) cover the baked-velocity limit, gated on Fire state + a B-press edge + cooldown. Validates [Approach A](docs/investigations/2026-05-26-spawn-template-forth-primitive.md) over the deferred `spawn-template` syscall. |
| 2026-05-26 | [Plan: Internet (WAN) voice + video via WebRTC](docs/plans/2026-05-26-internet-voice-video-webrtc.md) | **DONE 2026-05-26 (Phases 1–2); 3–4 deferred** | Took `wf-edit` voice/video over the internet via real WebRTC (libdatachannel) — encrypted DTLS-SRTP media, SDP/ICE signaling over a public `wss://` relay (new `CH_SIGNAL=0x05`), public STUN, staying P2P so server cost holds ~flat at scale. Deferred: Phase 3 TURN (coturn/Cloudflare Calls, potential revenue markup) + Phase 4 quick-tunnel demo UX. See [investigation](docs/investigations/2026-05-26-internet-voice-video-nat-traversal.md). |
...
|------|---------------|--------|---------|
| 2026-05-26 | [Internet (WAN) voice/video — NAT traversal & hosting](docs/investigations/2026-05-26-internet-voice-video-nat-traversal.md) | **Complete — recommends Option A** | Compares real WebRTC (libdatachannel), Rust WebRTC, hand-rolled ICE-lite, and server-relayed media for taking `wf-edit` calls over the internet, recommending libdatachannel (encrypted DTLS-SRTP, P2P with TURN fallback, server cost flat at scale), ruling out cleartext under the all-traffic-encrypted requirement, and answering the hosting questions (one small public signaling box; TURN deferred to coturn/Cloudflare Calls; quick-tunnel demo mode). |
| 2026-05-26 | [macOS port — runtime + engine delta estimate](docs/investigations/2026-05-26-macos-port-estimate.md) | **Estimate** | Two-pass macOS-port estimate — runtime (≈3 weeks) + editor/CRDT/collab delta (≈4 weeks); with no schedule pressure and iOS Metal finishing first, recommends targeting Metal directly (reusing the iOS backend) rather than shipping on GL and reworking later (~1.5–3.5 weeks of throwaway). |

...
| 2026-04-19 | [Plan: textile-rs validation & round-trip integration](docs/plans/2026-04-19-textile-rs-validation.md) | **In progress — Phase 1 first pass** | Phase 1 first pass (2026-04-19) landed the 16-bit BGR555 TGA fast-path fix (`bitmap.rs::try_load_tga_bgr555`): `image` crate doesn't support the 16-bit truecolor TGA variant WF uses natively, so every source texture was failing to load. Three remaining Phase 1 gaps identified: palette-atlas size mismatch (`palx=320` default vs oracle's `256`), textile-rs doesn't yet produce the `.bin` ASS-wrapped concatenations that levcomp-rs references, and `Room0`/`Perm` vs `rm0`/`perm` naming mismatch. Decisions pinned: byte-identical-vs-oracle bar; no SGI restoration; no C++ textile backport (oracle only). |
| 2026-04-19 | [Plan: levcomp-rs two-phase common-block emission](docs/plans/2026-04-19-levcomp-common-block-two-phase.md) | **Phase A + follow-ups done — 5 byte-diffs remain (99.8% closed)** | Four commits land the refactor plus three follow-up fixes: (a) `8e2f244` — two-phase emission + ObjectOnDisk heap-garbage type/rot pads (141 diffs); (b) `21ca707` — audit-fix 1, `field_str_child_only` accessor for I32 enum-label lookup (134 diffs; closed CamShot `Rotation`/`Position X/Y/Z` per obj[12]/obj[35]); (c) `0a37e20` — Actboxor01/02 OBJ-chunk swap in the `.lev` (83 diffs; closed the 40B object-data swap plus 11B room-entries cascade); (d) `88b9df7` — prepend `\n` to joystick-input `Script` STR in `snowgoons.lev` to match oracle byte-for-byte (5 diffs).  Remaining 5 bytes are the last unpredictable heap-garbage pads: 3B room-pad + 2B stray common-block bytes that no single mirror rule fixes consistently.  Full scorecard in the plan doc. |
...
**2026-04-19 13:31** — [`docs/plans/2026-04-19-levcomp-common-block-two-phase.md`](docs/plans/2026-04-19-levcomp-common-block-two-phase.md): Plan: levcomp-rs two-phase common-block emission — oracle byte-identity for the LVL chunk

...
- **SMB `?`-block bump mechanic parked (2026-05-17)** — Decided not to implement bump-from-below right now; the obvious "per-block Forth poll" is `O(N_blocks)` per tick and doesn't scale to a 1000-block level, and the scalable path (engine `Actor::Collision()` callback + a new `OnBumpFromBelowMailbox` actor field) is the kind of OAS-field-add we defer until after the first level ships. Architecture decision + ruled-out options captured in the [parked plan](docs/plans/2026-05-17-smb-qblock-bump-parked.md) so future-us doesn't re-explore.

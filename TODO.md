...
- [ ] **macOS: `-fullscreen`, `-width=N`, `-height=N` window flags — AppKit half.** The flags are parsed on macOS and feed the projection aspect (`hal/macos/display_macos.cc`), and `main.cc` no longer eats `-height=` (2026-09-19); what's left is `NSWindow` frame sizing + `toggleFullScreen:` / `NSWindowStyleMaskFullScreen`, which needs the macOS Metal window to exist first (see the Metal renderer item). Tablets (Android/iOS) are always fullscreen by OS design. [plan](docs/plans/2026-06-04-window-size-parity-fullscreen-flag.md)

...
- [ ] **Editor Phase 0b — engine embed-readiness (frame-step + external GL context + input injection).** Three surgical changes, ~3 weeks total. (1) Split `WFGame::RunLevel`'s `while` body into `Level::Step(dt)` / `WFGame::StepFrame(dt)` so the loop can move to the editor (~1–2 wks; [game/game.cc:256](wfsource/source/game/game.cc)). (2) Add `InitWithExistingContext(Display*, Window, GLXContext)` to [gfx/gl/mesa.cc:117](wfsource/source/gfx/gl/mesa.cc) that skips `XOpenDisplay` / `glXCreateContext` / `XCreateWindow`; standalone path stays for `wf_game` (~1 wk). (3) Expose a setter for `_joystickButtons` in mesa.cc so editor-owned events feed the engine directly (~2 days). Trigger: file-watch editor prototype gives way to direct-CRDT-read. Findings in [docs/investigations/2026-05-18-collaborative-level-editor-design.md](docs/investigations/2026-05-18-collaborative-level-editor-design.md) § Tier 1.

...
- [ ] **Editor Phase 0b — engine embed-readiness (frame-step + external GL context + input injection + de-global WFGame).** Three surgical changes outstanding; #3 + #4 already done (`b0639c5`, `1a957f7` + `89bcb58`). (1) [Split `WFGame::RunLevel` into `StepFrame`](docs/plans/2026-05-18-engine-frame-step-api.md) — ~1–2 wks; [game/game.cc:256](wfsource/source/game/game.cc). (2) [External `XDisplay*` / `Window` / `GLXContext` via `InitWithExistingContext`](docs/plans/2026-05-18-engine-external-gl-context.md) — ~1 wk; [gfx/gl/mesa.cc:117](wfsource/source/gfx/gl/mesa.cc); standalone path stays for `wf_game`. Independent plans; either can land first. Trigger: file-watch editor prototype gives way to direct-CRDT-read. Findings in [docs/investigations/2026-05-18-collaborative-level-editor-design.md](docs/investigations/2026-05-18-collaborative-level-editor-design.md) § Tier 1.

...
- [ ] **HAL pool allocators don't guarantee 8-byte alignment.** UBSan run 2026-05-19 (cmake build with `-DWF_ASAN=ON` + `-fsanitize=address,undefined`) flagged dozens of misaligned-access errors in `lmalloc.cc`, `dmalloc.cc`, `mempool.cc`, `_mempool.h` plus downstream constructors for `WFGame`, `Actor`, `Room`, and `FreeChunk` — the allocators hand out pointers whose alignment doesn't match the C++ types they back. Benign in practice on x86 (which tolerates misalignment with a 1-cycle penalty), but a correctness bug for SSE/AVX intrinsics, `std::atomic`, and any future ARM port that's stricter about alignment. Also a perf hit (every Actor field access pays the unaligned-load tax). Likely fix: round `LMalloc::_currentFree` and `DMalloc` chunk boundaries up to 8 bytes (16 if we want SSE-friendly) during allocation. ASan output for the full warning list saved in `/tmp/hostgl_asan2.log` during the 2026-05-19 snowgoons chase. Cross-ref: [BUGS.md](docs/BUGS.md) "`BaseObjectIterator` missing virtual destructor" — same ASan run.

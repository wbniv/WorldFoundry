...
- [ ] **Fix the CamShot `Rotation`/`Position` docs to match BungeeCam.** `docs/level-building.md` § "CamShot tracking toggles" and `docs/investigations/2026-04-29-camera-system.md` say `Fixed` makes the orientation static and ignores `Track Object`; it doesn't. Verified in `movecam.cc` + frames (condo plan § 5b): `Relative` → position = (camshot − Follow) + Track Object, `Absolute` → the camshot's point; direction = Target − camshot (world); `Track` only adds the tracked object's heading. **The missing doc is the collision climb**: the camera is a physics body — any bbox overlap with a `Mass > 0` actor (`Actor::CanCollide`) stops Z seeking and climbs (`ClimbRate`) until |Δz| ≥ the target range, which is why every shot placed inside a room sailed upward and why the sky dome needs `Mass 0`. Also `INDEXOF_CAMSHOT` is never cleared in bungee mode, and `ActBoxOR` needs `MovementClass 17`. Write the formulas + the climb rule into both docs. [plan](docs/plans/2026-09-19-condo-site-skybox.md)

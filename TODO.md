...
- [ ] **Recorder: wall-clock-pace the `-record_video` pipe.** It declares `-framerate 30` and writes one frame per rendered frame, so any run below 30 fps (e.g. 1280×960 ≈ 21 fps) plays fast. Duplicate/drop frames against `gettimeofday` in `CaptureFrame` (`gfx/gl/display.cc`), `WF_CAPTURE_RAW=1` to opt out. Drafted 2026-09-19, not applied. [plan](docs/plans/2026-09-19-window-size-flags-reach-the-game-parser.md)
...
- [wip] **Condo site skybox + 6th‑floor elevation** — OSM‑rendered sky dome + ground map for the Sathu Pradit site (back faces west), units lifted to z = 15.75 m, corridor outside the doors, tour video regenerated. [plan](docs/plans/2026-09-19-condo-site-skybox.md)

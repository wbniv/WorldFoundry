#!/usr/bin/env python3
"""Patch wflevels/snowgoons.iff.txt with Wren (//wren sigil) scripts.
...
Usage:
    python3 scripts/patch_snowgoons_wren.py [iff.txt] [iff.out]

Defaults: wflevels/snowgoons.iff.txt → wflevels/snowgoons.iff

Scripts use the `//wren` sigil; ScriptRouter dispatches to wren_engine
(before the generic `//` JS check).  Mailbox access uses the foreign class
Env (defined by wren_engine's preamble).  INDEXOF_* constants are available
as module-level vars injected by wren_engine::AddConstantArray.
"""
...
PLAYER_WREN = (
    b'//wren\n'
    b'Env.write_mailbox(INDEXOF_INPUT, Env.read_mailbox(INDEXOF_HARDWARE_JOYSTICK1_RAW))\n'
)
...
DIRECTOR_WREN = (
    b'//wren\n'
    b'var v\n'
    b'v = Env.read_mailbox(100)\n'
    b'if (v != 0) Env.write_mailbox(INDEXOF_CAMSHOT, v)\n'
    b'v = Env.read_mailbox(99)\n'
    b'if (v != 0) Env.write_mailbox(INDEXOF_CAMSHOT, v)\n'
    b'v = Env.read_mailbox(98)\n'
    b'if (v != 0) Env.write_mailbox(INDEXOF_CAMSHOT, v)\n'
)
...
def main() -> int:
    ...
    assert len(PLAYER_WREN)   <= SLOT_SIZE, "player script too long"
    assert len(DIRECTOR_WREN) <= SLOT_SIZE, "director script too long"

    text = txt_path.read_text()
    text = replace_script_block(text, 'DIRECTOR', DIRECTOR_WREN)
    text = replace_script_block(text, 'PLAYER',   PLAYER_WREN)

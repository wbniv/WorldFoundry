...
| 2026-04-19 | [Plan: levcomp-rs two-phase common-block emission](docs/plans/2026-04-19-levcomp-common-block-two-phase.md) | **Not started** | levcomp-rs's own `.lvl` output is 4 bytes short of oracle (8624 vs 8628) and only ~1352/3632 common-block bytes byte-identical, so `wflevels/snowgoons/snowgoons.lvl` is currently an oracle copy rather than levcomp-rs output.  Root cause per iff2lvl code review: iff2lvl runs two ordered phases (all-objects XDATA via `AddXDataToCommonData`, then all-objects per-object COMMONBLOCK via `CreateCommonData`); we do a single-per-object pass that interleaves, so the greedy dedup scan finds different matches.  Plan: refactor `oad_loader`/`lvl_writer` into Pass 1 (precompute XDATA offsets into a side table, seed common area) + Pass 2 (walk entries normally, reading the side table for XDATA refs), then `cmp` against `/tmp/oracle_lvl_payload.bin` for byte-identity — ~half a day. |

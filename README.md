...
**Level pipeline proof (2026-04-17, in progress)** — Phases A+B done: `primitives.lev` and `whitestar.lev` both compile through `iffcomp-rs` → `levcomp-rs`, and `wf_oad` has a `common.oad` fixture test. Remaining C–E (decompile subcommand, 4 source-less levels, multi-level `cd.iff`) gate any breaking `common.inc` rearrangement — which is what the deferred ScriptLanguage OAD plan needs.
...
| 2026-04-16 | [Plan: Android port](docs/plans/2026-04-16-android-port.md) | **In progress — Phases 1+2 done; Phase 0 steps 1/2/4a/4b/4c(a+b) done; Phase 3 steps 1–6 done** | Phase 1 ✅, Phase 2 ✅. Phase 0 🟡: modern VBO+shader backend renders snowgoons at visual parity with legacy (4c (a) per-vertex normals + (b) directional lighting confirmed 2026-04-17); 4c (c) fog / (d) rendmatt quads / (e) strip vestigials / (f) retire legacy remaining. Phase 3 🟡: `libwf_game.so` links; NativeActivity + EGL 3.0 live; Gradle project scaffolded; gamepad + touch input wired; `AAssetManager` reads `cd.iff` from APK; modern backend selected on Android. Step 7 (phone + Google TV device smoke test) remaining. |
...
| 2026-04-15 | [LOC tracking](docs/investigations/2026-04-15-loc-tracking.md) | **Ongoing** | Tracks code line count over time. Current HEAD: ~37,926 lines (−41.0% from baseline 64,252 at `74d1a47`; low-water mark was 36,199 / −43.7% at `e2dcc98` after Batch 7, then new value added back — Jolt, Android HAL, audio backend, gfx renderer seam). Tool: `scripts/loc_report.py`. |

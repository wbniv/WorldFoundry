...
| 2026-04-19 | [Plan: textile-rs validation & round-trip integration](docs/plans/2026-04-19-textile-rs-validation.md) | **Not started** | Rust port at `wftools/textile-rs/` exists and builds (~1.8K LOC, 3 commits on 2026-04-14, CLI-parity with C++) but has zero tests and no byte-identity claim against the oracle. Plan: build snowgoons harness (author `snowgoons.ini`, extract oracle PERM/RM1 atlases via iffdump-rs), run textile-rs, close byte-diffs (likely BGR555 quantization, palette dedup, allocmap ordering), wire into Blender round-trip to retire `swap_lvl.py`'s texture dependency, and pin as regression test. Decisions: byte-identical-vs-oracle bar; no SGI restoration; no C++ textile backport (oracle only). |

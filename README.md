...
**Audio (Phases 1–5 complete) (2026-04-17)** — Phase 1: miniaudio v0.11.25 vendored; `SoundDevice`/`SoundBuffer` reimplemented on `ma_engine`; `_InitAudio`/`_TermAudio` in HAL; fire-and-forget `play()` via heap `PlayInstance`. Phase 2: TinySoundFont (`tsf.h`/`tml.h`) vendored; `MusicPlayer` renders MIDI → stereo float PCM as a miniaudio custom data source on the audio thread; C-major scale audible via PulseAudio. Phase 3: per-level `level<N>.mid` loads on `RunLevel`, stops on exit; `level0.mid` (Für Elise) added for snowgoons. Phase 4: `play_music`/`stop_music`/`set_music_volume` Lua C closures registered in `scripting_lua.cc` — **Lua-only; not mailbox-wired, so the other seven scripting engines can't currently trigger audio**. Phase 5: `SoundBuffer::play(x,y,z)` positional overload; `SoundDevice::tick()` takes listener forward/up; `Level::updateSound()` feeds real camera transform each frame. Three miniaudio gotchas surfaced and were fixed in `d1e913d` (never `ma_sound_uninit` from the end callback — callback runs on the audio thread; push to a lock-free done-list and drain from `SoundDevice::tick()` instead. Zero listener dir/up silences spatialization — length-check first. Default `min_distance=1` is too aggressive for meter-scale worlds — set to 5). Verified in snowgoons: Für Elise + REF + panned right/left beeps all audible. Phases 6–7 (mobile backends, docs) deferred; mailbox-wired audio API for non-Lua engines also deferred.

**Android port (Phases 1+2 complete; Phase 0 partial; Phase 3 steps 1–6 done) (2026-04-17)** — **Phase 1** — `CMakeLists.txt` at repo root translates `build_game.sh` for both Linux and Android (NDK r26c, arm64-v8a, API 21); Linux CMake binary launches snowgoons; Forth-only flag combination (`WF_LUA_ENGINE=none`, no JS/WASM/Wren/Steam/REST) compiles. `task build-cmake` + `task build-cmake-android` + `task dev-setup` + `task android-sdk-install` (`63e016a`, idempotent) added. `cf_android.h` platform config defines `__LINUX__` so the 40+ downstream `#if defined(__LINUX__)` guards work without per-file patching; `ANDROID` enum member renamed `MACHINE_ANDROID` to avoid NDK macro collision. 64-bit pointer-truncation bug in `DispatchCollisionMessages` surfaced and fixed (`SMsg::_data._message` widened to `uintptr_t`). **Phase 2** — HAL lifecycle seam (`HALNotifySuspend`/`Resume`/`IsSuspended`); `AssetAccessor` POSIX backend reads `cd.iff` through a swappable seam. **Phase 3** — Step 1 (`8838aef`): `hal/android/` stubs + stub silent audio + `android_window.cc` peer of `mesa.cc`, `libwf_game.so` links ~30 MB arm64-v8a. Step 2 (`3421125`): `android_main` on glue thread, real EGL 3.0 init from `ANativeWindow` (`eglChooseConfig(RGB888,D16,GLES3_BIT)`, `eglCreateContext(client=3)`), ALooper drain each frame. Step 3 (`3b24f00`): Gradle project at `android/` (AGP 8.5.2, compileSdk 34, minSdk 21, arm64-v8a only, `externalNativeBuild` → repo-root `CMakeLists.txt`); `AndroidManifest.xml` NativeActivity with LAUNCHER + LEANBACK_LAUNCHER intent-filters, landscape-locked; `task build-apk`/`install-apk`. Step 4a (`148b319`): gamepad — AKEYCODE_DPAD_* / BUTTON_A-Y / L1/R1 + joystick AXIS_X/Y/HAT_X/HAT_Y threshold → `EJ_BUTTONF_*`. Step 4b (`58fd790`): touch — fixed-pixel bottom-left d-pad cross + bottom-right A/B rectangles, multi-touch merged, `AConfiguration_getUiModeType` suppresses touch on TV. Step 5: `AAssetManager` accessor reads `cd.iff` from APK via symlink to canonical `wfsource/source/game/cd.iff` (no copy, no staleness). Step 6: modern VBO+shader backend routes via Phase 0 step 4b's `backend_factory.cc`. Step 7 remaining: phone + Google TV device smoke test.

**Blender ↔ level round-trip (2026-04-17)** — `levcomp-rs` (Rust) compiles `.lev` source → binary `.lvl`; snowgoons loads end-to-end from `levcomp-rs` output. Blender plugin imports/exports 152/152 OAD fields, all light/slope/animation-channel types, Tcl→Lua script migration. Coordinate system fixed (WF is Z-up). Phase 2c complete: `mesh_bbox.rs` extracts per-mesh bounding boxes from MODL/VRTX chunks; `asset_registry.rs` assigns packed `[TYPE|ROOM|INDEX]` asset IDs and emits `asset.inc`; `ButtonType::Filename` handled alongside `MeshName`; 37 objects placed, all Jolt bodies valid. Remaining: real path/channel keyframes.

**Tooling and plans (2026-04-17)** — `engine/` reorganised to top-level. REST API box PoC landed. iOS port plan written (blocked on Android). CLI level override (`-L<path>`) confirmed done. Audio investigation updated: `AudioBackend` pimpl seam documented; MIDI sources (OpenScore CC0, Mutopia, piano-midi.de) researched and catalogued; IFF format lineage (EA IFF 85 → AIFF, RIFF, WF IFF) documented.

**Scripting system (2026-04-16)** — Seven engines smoke-tested end-to-end in snowgoons: Lua 5.4, Fennel, QuickJS, JerryScript, WAMR (classic interp), Wren, and Forth (zForth). Five additional Forth backends (ficl, atlast, embed, libforth, pforth) build and link but are not yet end-to-end tested. All wired into a `ScriptRouter` dispatch table with per-engine sigils. wasm3 retired 2026-04-16 (WAMR reached parity). Lua made optional via `WF_LUA_ENGINE=lua54|none`; `scripting_lua.cc/hp` extracted as a peer TU matching all other engine plugs. WAMR AOT deferred.

**Dead-code removal (2026-04-15)** — Batches 1–7 complete: `wfsource/source/` reduced from 64,252 → 36,199 lines (−43.7%). Batch 8 (Jolt replaces WF physics) in progress.

**Jolt Physics (2026-04-14)** — Integrated and default (`WF_PHYSICS_ENGINE=jolt`). Five-step plan complete: SIGABRT fixed, zombie kinematic bodies eliminated, WF↔Jolt authority model locked, 3 m vertical pop fixed (feet vs centre offset), 60 s soak passed. Legacy `physics/wf/` retained pending a second level.

**Steam (Phases 1–2) (2026-04-12)** — Steamworks SDK lifecycle wired into HAL (`SteamAPI_Init/Shutdown` in `HALStart`) and display (`SteamAPI_RunCallbacks` after `XEventLoop` in `PageFlip`). Steam Input polls all connected controllers each frame and ORs results into `_JoystickButtonsF`; keyboard path unchanged. `WF_ENABLE_STEAM=1` build flag; SDK not committed (see `engine/vendor/steamworks/README.md`). `steam/game_actions_480.vdf` defines the `InGame` action set. `steam_appid.txt=480` for dev. Phases 3 (SteamPipe depot) and 4 (store page) deferred.
